...
SQLite migrations currently reach schema version 20. Schema 2 adds provider preset/adapter/enabled
...
optional bounded `organization` identifier. Both fields are validated as non-secret profile text,
persisted across reopen, and excluded from diagnostics; the organization identifier is forwarded only
as `OpenAI-Organization` for OpenAI Chat Completions and Responses requests.
