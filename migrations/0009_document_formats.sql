CREATE TABLE document_jobs_v9 (
    ...
    format TEXT NOT NULL CHECK (format IN ('txt', 'markdown', 'srt', 'webvtt', 'csv')),
    source_name TEXT NOT NULL,
    created_at INTEGER NOT NULL DEFAULT (unixepoch()),
    updated_at INTEGER NOT NULL DEFAULT (unixepoch())
);

INSERT INTO document_jobs_v9 (job_id, state, format, source_name, created_at, updated_at)
SELECT job_id, state, format, source_name, created_at, updated_at
FROM document_jobs;

CREATE TABLE document_segments_v9 (
    job_id TEXT NOT NULL REFERENCES document_jobs_v9(job_id) ON DELETE CASCADE,
...
INSERT INTO document_segments_v9 (job_id, segment_index, kind, source_text, translated_text, line_ending)
SELECT job_id, segment_index, kind, source_text, translated_text, line_ending
FROM document_segments;

CREATE TABLE document_job_options_v9 (
    job_id TEXT PRIMARY KEY REFERENCES document_jobs_v9(job_id) ON DELETE CASCADE,
    source_locale TEXT,
...
INSERT INTO document_job_options_v9 (job_id, source_locale, target_locale, model_id, provider_id, glossary_json, updated_at)
...
ALTER TABLE document_jobs_v9 RENAME TO document_jobs;
ALTER TABLE document_segments_v9 RENAME TO document_segments;
ALTER TABLE document_job_options_v9 RENAME TO document_job_options;
