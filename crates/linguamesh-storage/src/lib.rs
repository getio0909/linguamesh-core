...
const PROFILE_QUERY_BY_ID: &str = "SELECT p.id, p.display_name, p.preset_id, p.adapter_type, p.base_endpoint, p.secret_ref, p.user_notes, p.organization, p.project, p.region, p.account_identifier, p.custom_headers, p.secret_custom_headers_ref, p.proxy_url, p.request_timeout_secs, p.enabled, s.model_id FROM provider_profiles p LEFT JOIN provider_model_selection s ON s.provider_id = p.id WHERE p.id = ?1";
const PROFILE_QUERY_ALL: &str = "SELECT p.id, p.display_name, p.preset_id, p.adapter_type, p.base_endpoint, p.secret_ref, p.user_notes, p.organization, p.project, p.region, p.account_identifier, p.custom_headers, p.secret_custom_headers_ref, p.proxy_url, p.request_timeout_secs, p.enabled, s.model_id FROM provider_profiles p LEFT JOIN provider_model_selection s ON s.provider_id = p.id ORDER BY p.display_name, p.id";
const PROFILE_QUERY_ACTIVE: &str = "SELECT p.id, p.display_name, p.preset_id, p.adapter_type, p.base_endpoint, p.secret_ref, p.user_notes, p.organization, p.project, p.region, p.account_identifier, p.custom_headers, p.secret_custom_headers_ref, p.proxy_url, p.request_timeout_secs, p.enabled, s.model_id FROM active_provider_selection a JOIN provider_profiles p ON p.id = a.provider_id LEFT JOIN provider_model_selection s ON s.provider_id = p.id WHERE a.singleton = 1";
...
mod tests {
    ...
    fn migration_and_manual_selection_are_persistent() {
        ...
        assert_eq!(storage.schema_version().expect("version"), 26);
...
";
    ...
    fn trusted_descriptor_path_requires_proc_self_fd() {
        ...
        assert_eq!(storage.schema_version().expect("schema version"), 26);
        assert!(matches!(
